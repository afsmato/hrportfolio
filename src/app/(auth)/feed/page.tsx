...
async function ArticleList({ searchParams }: { searchParams: SearchParams }) {
    ...
    <>
      ...
      {totalPages > 1 && (
        <div style={{ display: 'flex', justifyContent: 'center', gap: '0.5rem', marginTop: '2rem' }}>
          {page > 1 && (
            <a href={`/feed?${new URLSearchParams({ ...(category ? { category } : {}), ...(difficulty ? { difficulty } : {}), page: String(page - 1) })}`}
  ...
  return (
    <main style={{ maxWidth: '48rem', margin: '0 auto', padding: '2rem 1rem' }}>
      ...
      <Suspense fallback={<p style={{ color: '#6b7280' }}>読み込み中...</p>}>
        <ArticleList searchParams={params} />
      </Suspense>
    </main>
  );
}
